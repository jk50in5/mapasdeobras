...
# - Persistência: CSV local (ephemero) OU Google Sheets (persistente)
...
# mapa/zoom
MAP_CENTER = (-8.50, -36.80); MAP_ZOOM = 7
RMR_BOUNDS = ((-8.50, -35.35), (-7.70, -34.75))
RECIFE_CENTER = (-8.05428, -34.8813)

# viewbox Nordeste para geocoder
...
def _gs_client():
    ...
    try:
        info = json.loads(svc_json)  # pode ser JSON string ou já vir como dict em secrets
...
def do_login_with_authenticator():
    ...
    for u, pw in USERS_ENV.items():
        ...
        if _is_bcrypt_hash(pw):
            ...
            # tenta hash novo; se não der, usa plaintext e ativa auto_hash
            try:
                hashed = stauth.Hasher.hash(pw)  # API >= 0.4.x
            except Exception:
                try:
...
if HAS_STAUTH:
    ...
    st.info("Login simples ativo (instale 'streamlit-authenticator' para segurança melhor).")
    ...
    if not st.session_state.auth_ok:
        ...
        if ok and u in USERS_ENV and USERS_ENV[u] == p:
            st.session_state.auth_ok = True; st.session_state.username = u; st.rerun()
        elif ok:
            st.error("Usuário/senha inválidos."); st.stop()
    else:
        with st.sidebar:
...
def make_geolocator():
    ...
    return {"geo": geo, "geocode": RateLimiter(geo.geocode, min_delay_seconds=1),
...
def geocode_resiliente(endereco: str, cidade: str|None=None, uf_hint: str|None=None):
    g = make_geolocator(); geocode = g["geocode"]
    cand=[]; endereco=_norm(endereco); cidade=_norm(cidade)
    if uf_hint: uf_hint=_norm(uf_hint).upper()
    if endereco:
    ...
    if cidade and uf_hint: cand.append(f"{cidade}, {uf_hint}, Brasil")
    if uf_hint: cand.append(f"{uf_hint}, Brasil")
    if not cidade:
        m=re.search(r",\s*([A-Za-zÀ-ú\s]+),\s*(PE|PB|AL)\b", endereco, flags=re.I)
        if m: cand.append(f"{m.group(1).strip()}, {m.group(2).upper()}, Brasil")
    seen=set(); queries=[]
    for q in cand:
        if q and q not in seen: queries.append(q); seen.add(q)
    for q in queries:
        try:
            loc = geocode(q, addressdetails=True, country_codes=COUNTRY,
                          viewbox=VIEWBOX_NE, bounded=True, timeout=10)
        except Exception:
            loc=None
        if loc:
...
def reverse_uf(lat: float, lon: float) -> str|None:
    g=make_geolocator(); rev=g["reverse"]
    try:
...
def in_bounds(lat: float, lon: float, bounds) -> bool:
    ...
    return (pd.notna(lat) and pd.notna(lon) and south<=lat<=north and west<=lon<=east)
...
def next_id(df: pd.DataFrame) -> int:
    ...
    try: return int(df["id"].astype(int).max()) + 1
    except Exception: return len(df) + 1
...
if limpar_btn: st.experimental_rerun()
...
if add_btn:
    ...
    if not nome_obra.strip() or not endereco.strip():
        ...
        if m: uf_in_text = m.group(1)

        if lat_m is not None and lon_m is not None:
            uf = reverse_uf(lat_m, lon_m) or (uf_in_text or "")
            novo = {
                "id": next_id(df), "nome_obra": nome_obra.strip(), "contato": contato.strip(),
                "telefone": telefone.strip(), "endereco": endereco.strip(), "cidade": _norm(cidade),
                "estado": uf, "status": status_label, "lat": lat_m, "lon": lon_m,
                "criado_em": datetime.now().strftime("%Y-%m-%d %H:%M")
            }
            df = pd.concat([df, pd.DataFrame([novo])], ignore_index=True)
            st.session_state.df = df; save_data(df)
            ...
            if geo:
                novo = {
                    "id": next_id(df), "nome_obra": nome_obra.strip(), "contato": contato.strip(),
                    "telefone": telefone.strip(), "endereco": endereco.strip(), "cidade": _norm(cidade),
                    "estado": geo["uf"] or (uf_in_text or ""), "status": status_label,
                    "lat": geo["lat"], "lon": geo["lon"], "criado_em": datetime.now().strftime("%Y-%m-%d %H:%M")
                }
                df = pd.concat([df, pd.DataFrame([novo])], ignore_index=True)
                st.session_state.df = df; save_data(df)
                ...
                st.session_state.pending = {
                    "id": next_id(df), "nome_obra": nome_obra.strip(), "contato": contato.strip(),
                    "telefone": telefone.strip(), "endereco": endereco.strip(), "cidade": _norm(cidade),
                    "estado": uf_in_text or "", "status": status_label,
                    "lat": None, "lon": None, "criado_em": datetime.now().strftime("%Y-%m-%d %H:%M")
                }
...
with cols[3]:
    st.download_button("Baixar dados (.csv)",
        data=st.session_state.df.to_csv(index=False).encode("utf-8"),
        file_name="obras.csv", mime="text/csv", use_container_width=True)

df_view = st.session_state.df.copy()
if filtro_status: df_view = df_view[df_view["status"].isin(filtro_status)]
if filtro_uf:     df_view = df_view[df_view["estado"].isin(filtro_uf)]

def filter_rmr(df_in: pd.DataFrame) -> pd.DataFrame:
    if df_in.empty: return df_in
    return df_in[df_in.apply(lambda r: in_bounds(r.get("lat"), r.get("lon"), RMR_BOUNDS), axis=1)]
...
if st.session_state.await_click and st.session_state.pending and st_data:
    ...
    if last and "lat" in last and "lng" in last:
        ...
        novo["lat"] = latc; novo["lon"] = lonc; novo["estado"] = uf
        st.session_state.df = pd.concat([st.session_state.df, pd.DataFrame([novo])], ignore_index=True)
        save_data(st.session_state.df)
        st.session_state.pending = None; st.session_state.await_click = False
...
with st.sidebar.expander("Gerenciar alfinetes (editar/excluir)", expanded=False):
    ...
    if dfe.empty:
        ...
        opcoes = dfe.assign(
            rotulo=lambda d: d.apply(lambda r: f"{r['id']} — {r.get('nome_obra','(sem nome)')} ({r.get('estado','')})", axis=1)
        )[["id","rotulo"]]
        ...
        with st.form("form_editar", clear_on_submit=False):
            ...
            edit_end = st.text_area("Endereço (rua, nº, bairro, cidade, UF) *", value=row.get("endereco",""))
            edit_city = st.text_input("Cidade", value=row.get("cidade",""))
            status_idx = status_labels.index(row.get("status","Obra não visitada")) if row.get("status","Obra não visitada") in status_labels else 0
            ...
            confirm_id = colE2.text_input("Confirmar ID p/ excluir", value="", help=f"Digite {sel_id} para excluir.")
...
with st.expander("Ver dados (visualização)"):
    st.dataframe(st.session_state.df.sort_values("criado_em", ascending=False), use_container_width=True)
